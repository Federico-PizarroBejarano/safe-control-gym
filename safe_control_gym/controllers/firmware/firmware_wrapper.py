...
class FirmwareWrapper(BaseController):
    ...
    CONTROLLER = 'pid'  # specifies controller type
    ...
    def sendCmdVel(self, roll, pitch, yaw, thrust, timestep):
        '''Adds a sendcmdvel command to command processing queue.

        Notes:
            Overrides any high level commands being processed.

        Args:
            roll (float): roll / roll rate of the CF (rad).
            pitch (float): pitch / pitch rate of the CF (rad).
            yaw (float): yaw / yaw rate of the CF (rad/s).
            thrust (float): thrust of the CF, non-meaningful units in [0, 2^16),
                            where the maximum value corresponds to maximum thrust.
            timestep (float): simulation time when command is sent (s).
        '''
        self.command_queue += [['_sendCmdVel', [roll, pitch, yaw, thrust, timestep]]]

    def _sendCmdVel(self, roll, pitch, yaw, thrust, timestep):
        # print(f"INFO_{self.tick}: CmdVel command sent.")
        yawMode = YawModeType.XMODE  # Default: XMODE

        thrustLocked = False  # Default: True
        altHoldMode = True  # Default: False
        posHoldMode = False  # Default: False
        posSetMode = False  # Default: False

        stabilizationModeRoll = RPYType.ANGLE  # Default: ANGLE
        stabilizationModePitch = RPYType.ANGLE  # Default: ANGLE
        stabilizationModeYaw = RPYType.ANGLE  # Default: RATE

        MIN_THRUST = 1000
        MAX_THRUST = 60000

        if not posSetMode:
            roll = roll * self.RAD_TO_DEG
            pitch = -pitch * self.RAD_TO_DEG
            yaw = yaw * self.RAD_TO_DEG
        ...
        # Thrust
        if thrustLocked or thrust < MIN_THRUST:
            self.setpoint.thrust = 0
        else:
            self.setpoint.thrust = min(thrust, MAX_THRUST)

        if altHoldMode:
            self.setpoint.thrust = 0
            self.setpoint.mode.z = firm.modeVelocity

            self.setpoint.velocity.z = thrust
        else:
        ...
        if posHoldMode:
            ...
            self.setpoint.velocity.x = pitch / 30.0
            self.setpoint.velocity.y = roll / 30.0
            self.setpoint.attitude.roll = 0
            self.setpoint.attitude.pitch = 0
        elif posSetMode and thrust != 0:
            self.setpoint.mode.x = firm.modeAbs
            self.setpoint.mode.y = firm.modeAbs
            self.setpoint.mode.z = firm.modeAbs
            self.setpoint.mode.roll = firm.modeDisable
            ...
            self.setpoint.position.x = -pitch
            self.setpoint.position.y = roll
            self.setpoint.position.z = thrust / 1000.0

            self.setpoint.attitude.roll = 0
            self.setpoint.attitude.pitch = 0
            self.setpoint.attitude.yaw = yaw
            self.setpoint.thrust = 0
        else:
            self.setpoint.mode.x = firm.modeDisable
            self.setpoint.mode.y = firm.modeDisable

            if stabilizationModeRoll == RPYType.RATE:
                self.setpoint.mode.roll = firm.modeVelocity
                self.setpoint.attitudeRate.roll = roll
                self.setpoint.attitude.roll = 0
            else:
            ...
            if stabilizationModePitch == RPYType.RATE:
                self.setpoint.mode.pitch = firm.modeVelocity
                self.setpoint.attitudeRate.pitch = pitch
                self.setpoint.attitude.pitch = 0
            else:
        ...
        # Yaw
        if not posSetMode:
            if stabilizationModeYaw == RPYType.RATE:
                # legacy rate input is inverted
                self.setpoint.attitudeRate.yaw = -yaw
                self.yawModeUpdate(yawMode)
    ...
    def yawModeUpdate(self, yawMode):
        '''Update Yaw according to current setting.

        Args:
            yawRad (float): Amount of radians to rotate yaw.
        '''
        if yawMode == 'CAREFREE':
            # TODO: Add frame of reference to setpoint
            assert (False)
        elif yawMode == 'PLUSMODE':
            self.rotateYaw(45 * np.pi / 180)
        elif yawMode == 'XMODE':
            pass
        else:
            pass

    def rotateYaw(self, yawRad):
        ...
        originalRoll = self.setpoint.attitude.roll
        originalPitch = self.setpoint.attitude.pitch

        self.setpoint.attitude.roll = originalRoll * cosy - originalPitch * siny
        self.setpoint.attitude.pitch = originalPitch * cosy + originalRoll * siny
...
class Command(Enum):
    ...
    CMDVEL = 7  # Args: [roll, pitch, yaw, thrust]
    # https://crazyswarm.readthedocs.io/en/latest/api.html#pycrazyswarm.crazyflie.Crazyflie.cmdVel
